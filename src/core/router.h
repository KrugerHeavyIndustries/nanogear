...
public:
     ...
     */
    void attach(const std::string& path, Resource* resource)
    { m_routes[path] = resource; }

    /*!
     * \return A modifiable QHash representing the routes
     */
   std::unordered_map<std::string, Resource*>& routes()
    { return m_routes; }

private:
    std::unordered_map<std::string, Resource*> m_routes;
   
    Representation m_notFound;
};
