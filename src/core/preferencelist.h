...
#ifndef PREFERENCELIST_H
#define PREFERENCELIST_H
...
template <typename T> class PreferenceList : public std::vector< Preference<T> >
{
public:
    /*!
    ...
    PreferenceList(const std::vector< Preference<T> >& other)
            :  std::vector< Preference<T> >(other) {}
    ...
    std::map<float, T> toMap() const {
       std::map<float, T> ret;
       for (typename PreferenceList<T>::const_iterator i = PreferenceList<T>::begin();
            i != PreferenceList<T>::end(); ++i)
       {
          ret.insert(std::make_pair(i->quality(), i->data()));
       }
          
       //for (template <> PreferenceList<T>::const_iter i = this->begin(); i != this->end(); ++i) {
