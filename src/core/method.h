...
public:
	...
	Method(const std::string& name) : Metadata(name) {}

    Method(const std::string& name, const std::string& description) : Metadata(name, description) {}

    bool operator==(const Method& type) const;

    /*!
     * Retrieve an instance of a Method class from the method's name
     *
     * \param methodName Requested method name
     * \return An instance of the default methods provided, otherwise a new Method instance with
     *      methodName as name (normalized to upper case)
     */
    static Method valueOf(const std::string& methodName);
