...
void MimeType::fromString(const std::string& type)
{
    m_whole = type;
    m_type.clear();
    m_subtype.clear();
...
{
    if (m_type.empty())
        const_cast<MimeType*>(this)->m_type = m_whole.substr(0, m_whole.find_first_of('/'));
...
{
    if (m_type.empty())
        const_cast<MimeType*>(this)->m_subtype = m_whole.substr(m_whole.find_first_of('/'));
