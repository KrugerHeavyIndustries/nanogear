 ...
 *
 * DirectoryResource can be used to provide filesystem access to the clients.
 *
 ...
 *
 * DirectoryResource accepts a directory as the 'root' directory and every file
 * contained will be mapped to an URI path.
 */
class DirectoryResource : public Resource
{
public:
	DirectoryResource(const std::string& root);
...
private:
     ...
     * \param response A reference to the Response object passed to handleGet
     */
    void representFile(const nanogear::File& pathInfo, Response& response);
    ...
    Representation m_notAllowed;
    Representation m_directoryIndex;
    Representation m_rawFile;
};
